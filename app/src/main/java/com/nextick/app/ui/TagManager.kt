...
object TagManager {
    ...
    fun show(fragment: Fragment, onSaved: () -> Unit) {
        ...
        fun addRow(source: Tag?, index: Int) {
            ...
            val name = EditText(ctx).apply {
                ...
                setText(
                    source?.let {
                        TagNames.of(ctx, it.nameKey, it.name)
                    } ?: ctx.getString(R.string.new_tag)
                )
                setSelectAllOnFocus(true)
                setOnFocusChangeListener { _, hasFocus ->
                    if (hasFocus) Notifier.tap(ctx)
                }
            }
            ...
            rowLayout.addView(
                swatch,
                LinearLayout.LayoutParams(ctx.dp(26), ctx.dp(26))
            )
    ...
    private fun save(ctx: android.content.Context, rows: List<Row>) {
        ...
        rows.forEach { row ->
            ...
            val key = source?.nameKey?.takeIf {
                TagNames.isDefaultName(ctx, it, name)
            }
